...
void addr_mode_fprint(addr_mode_t *addr_mode, uint32_t flags, FILE *fp) {
  ...
  switch(addr_mode->addr_mode) {
    case ADDR_MODE_MEM_DIRECT: {
      fprintf(fp, "0x%04X", addr_mode->direct_addr_16);
    } break;
...
ins_reader_t *ins_reader_init(const char *filename) {
  ...
  ins_reader->end = (uint8_t *)ins_reader->data + ins_reader->size;
...
void ins_reader_next(ins_reader_t *ins_reader, ins_t *ins) {
  ...
  ins_reader->ptr = parse_ins(ins, ins_reader->ptr);
  // Advance the address
...
void *parse_ins(ins_t *ins, void *data) {
  ...
  switch(ins->opcode) {
    ...
    case 0xA0: { // mov AL, [offset]
      ins->op = OP_MOV;
      operand_set_register(&ins->dest, REG_AL);
      data = operand_set_mem_direct_addr_16(&ins->src, data);
    } break;
    ...
    case 0xA2: { // mov [offset], AL
      ins->op = OP_MOV;
      data = operand_set_mem_direct_addr_16(&ins->dest, data);
