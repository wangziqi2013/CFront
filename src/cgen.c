...
// 4. If none storage class then by default it is definition even without init list
void cgen_global_decl(type_cxt_t *cxt, token_t *global_decl) {
  ...
  while(global_var) {
    ...
    if(type->size == TYPE_UNKNOWN_SIZE) 
      error_row_col_exit(decl->offset, "Incomplete type in definition of global variable\n");
